...
def peristimulus_histogram(waveform, stimulus_timing, fsample, binsize, spike_threshold):
    tspike=numpy.where(numpy.diff(numpy.where(waveform>spike_threshold, 1, 0))==1)[0]/fsample
    tstim=signal.detect_edges(stimulus_timing,threshold_otsu(stimulus_timing))/fsample
