...
void GenericStrobe()
{
	...
	if( IC_IsPropertyAvailable( hGrabber, "Strobe",NULL) == IC_SUCCESS )
	{
		...
		result = IC_SetPropertySwitch(hGrabber,"Trigger","Enable", 0);
		result = IC_SetPropertyAbsoluteValue(hGrabber,"Trigger","Debounce Time",100);
...
void Use_2_FrameFilters()
{
	...
	if( IC_CreateFrameFilter("Rotate Flip", &Rotate) == IC_SUCCESS &&
		IC_CreateFrameFilter("ROI", &ROI) == IC_SUCCESS )
	{
		...
		printf("Filter successfully created\n");
		hGrabber = IC_CreateGrabber();
		IC_OpenVideoCaptureDevice(hGrabber,"DFK 42AUC03");
		IC_AddFrameFilterToDevice(hGrabber, ROI);
		IC_AddFrameFilterToDevice(hGrabber, Rotate);


		IC_FrameFilterSetParameterInt(ROI,(char*)"Top",40);
        IC_FrameFilterSetParameterInt(ROI,(char*)"Left",120);
        IC_FrameFilterSetParameterInt(ROI,(char*)"Height",400);
        IC_FrameFilterSetParameterInt(ROI,(char*)"Width",400);

		IC_FrameFilterSetParameterInt(Rotate,"Rotation Angle", 90);
		...
		IC_RemoveFrameFilterFromDevice(hGrabber, Rotate );

		printf("Rotate Filter removed\n");
		...
		printf("Rotate Filter inserted again\n");
		...
		IC_DeleteFrameFilter( Rotate );
		IC_DeleteFrameFilter( ROI );
		IC_FrameFilterDeviceClear(hGrabber);
		Rotate.pFilter = NULL;
		ROI.pFilter = NULL;
		IC_ReleaseGrabber( &hGrabber);
...
void Exposure_Gain()
{
	...
	IC_SetPropertyAbsoluteValue(hGrabber,"Exposure","Value",0.0303);
	...
	if( IC_GetPropertyValueRange(hGrabber,"Contrast","Value",&Min, &Max ) == IC_SUCCESS)
	{
		IC_GetPropertyValue(hGrabber,"Contrast","Value",&Value);
		IC_SetPropertyValue(hGrabber,"Contrast","Value",-3);
	}

	if( IC_GetPropertyValueRange(hGrabber,"Hue","Value",&Min, &Max ) == IC_SUCCESS)
	{
		IC_GetPropertyValue(hGrabber,"Hue","Value",&Value);
		IC_SetPropertyValue(hGrabber,"Hue","Value",-3);
	}
...
void Using_Y800()
{
	...
	IC_SetFormat(hGrabber,Y800 );		// Set memoryformat in the sink YUVY
	...
	if( IC_SnapImage(hGrabber, 200) == IC_SUCCESS )
	{
		...
		pImageData = IC_GetImagePtr(hGrabber );
		IC_SaveImage(hGrabber, "b:\\test.bmp",FILETYPE_BMP,0);
	}
	printf("Press a key to continue\n");
	_getch();
	IC_StopLive(hGrabber);
...
void ListAvailableFrameRates()
{
	...
	printf("List available frame rates.\n");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Callback for image saving. Image counter is passed in Data and incremented here.
void  _cdecl SaveImageCallback(HGRABBER hGrabber, unsigned char* pData, unsigned long frameNumber, void* Data)
{
	int* pImageCounter;
	char Filename[255];
	pImageCounter = (int*)Data;

	(*pImageCounter)++;
	sprintf(Filename,"Img%04d.jpg",*pImageCounter);

	IC_SaveImage(hGrabber, Filename,FILETYPE_JPEG,90); 
	printf("Image %s saved.\n",Filename);
}


void SaveImageOnTrigger()
{
	HGRABBER hGrabber; // The handle of the grabber object.
	int ImageCounter;
	ImageCounter = 0;
	...
	if( hGrabber )            
	{
		IC_SetFrameReadyCallback (hGrabber,   *SaveImageCallback,  (void*)&ImageCounter);
		IC_SetContinuousMode(hGrabber,0); // Save every image automatically and call the callback.
		IC_EnableTrigger(hGrabber,1);
		IC_StartLive(hGrabber,1);
		...
		IC_StopLive( hGrabber );
		IC_EnableTrigger(hGrabber,0); // Clean up..
		IC_ReleaseGrabber( &hGrabber ); 
	}
}
...
void LoadDevice()
{
	...
	hGrabber = IC_ShowDeviceSelectionDialog(NULL); // Show the built in device select dialog
	if( IC_LoadDeviceStateFromFileEx(hGrabber,"device.xml",0) == IC_SUCCESS )
...
void testing()
{
	...
	hGrabber = IC_CreateGrabber();
	IC_OpenVideoCaptureDevice(hGrabber, "DFK 33GX174");
	IC_SetVideoFormat(hGrabber, "RGB32 (1920x1200)");
	IC_SetFrameRate(hGrabber, 50.0f);
	IC_StartLive(hGrabber, 1);
	IC_IsLive(hGrabber);


	printf("Press any key to continue to end live video\n");
	_getch();
