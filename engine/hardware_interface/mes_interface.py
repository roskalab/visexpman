...
def generate_paths(filename, config):
    path = utils.generate_filename(os.path.join(config.EXPERIMENT_DATA_PATH, filename))
...
class MesInterface(object):
    ...
    '''
    def __init__(self, config, connection, screen = None, log = None, ):
        self.config = config
    ...
    def acquire_z_stack(self, timeout = -1, channel = 'pmtUGraw', test_mat_file = None):
        z_stack_path, z_stack_path_on_mes = generate_paths('z_stack.mat', self.config)    
        utils.empty_queue(self.response_queue)
        results = []
        #Acquire z stack        
        if self.connection.remote_client_status:
            self.command_queue.put('SOCacquire_z_stackEOC{0}EOP' .format(z_stack_path_on_mes))            
            results.append(network_interface.wait_for_response(self.response_queue, 'SOCacquire_z_stackEOCstartedEOP', error_response = 'error', timeout = timeout))
            if results[-1] != 'timeout':
                results.append(network_interface.wait_for_response(self.response_queue, ['SOCacquire_z_stackEOCOKEOP', 'SOCacquire_z_stackEOCUSEOP'], error_response = 'error', timeout = -1))
                results.append(network_interface.wait_for_response(self.response_queue, 'SOCacquire_z_stackEOCsaveOKEOP', error_response = 'error', timeout = -1))
            else:
            ...
            if isinstance(z_stack_path, str):
                if os.path.exists(z_stack_path):            
                    z_stack = self.z_stack_from_mes_file(z_stack_path, channel = channel)
            return z_stack, results
        else:
            return {}, results

    def z_stack_from_mes_file(self, mes_file, channel = 'pmtUGraw'):
        '''
    ...
    def rc_scan(self, trajectory, timeout = -1):
        ...
        rc_scan_path, rc_scan_path_on_mes = generate_paths('rc_scan.mat', self.config)
        utils.empty_queue(self.response_queue)
        if self.connection.remote_client_status:
    ...
    def set_trajectory(self, points, timeout = -1):
        points_mat, points_mat_on_mes = generate_paths('rc_points.mat', self.config)
        self.generate_scan_points_mat(points, points_mat)
        utils.empty_queue(self.response_queue)
    ...
    def start_line_scan(self, mat_file_path):
        aborted = False
        self.stop = False
        if self.connection.remote_client_status or True:
