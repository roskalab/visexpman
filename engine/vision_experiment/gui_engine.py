...
#            self.to_gui.put({'display_trace_parameters':parameters[0]})
    ...
    def merge_datafiles(self,src=None,dst=None):
        '''
        Roi/repetition info is taken from src, and added to dst
        Use case: files regenerated by file converter and already created rois are copied to those files
        #self.engine.merge_datafiles()
        '''
        src='q:\\Rei\\20161123 P198-GCaMP6s rectangles'
        dst='q:\\Zoltan\\20161123 P198-GCaMP6s rectangles'
        files=os.listdir(src)
        files.sort()
        for f in files:
            if os.path.splitext(f)[1]!='.hdf5': continue
            f=os.path.join(src,f)
            targetf=os.path.join(dst,os.path.basename(f))
            self.printc(targetf)
            if os.path.exists(targetf.replace('.hdf5','.mat')): continue
            h=hdf5io.Hdf5io(targetf)
            h.rois=hdf5io.read_item(f,'rois',filelocking=False)
            h.repetition_link=hdf5io.read_item(f,'repetition_link',filelocking=False)
            if h.rois==None:
                h.close()
                self.printc('Skipped')
                continue
            else:
                h.save(['rois','repetition_link'])
            h.close()
            self.open_datafile(targetf)
            self.readd_rois(targetf)
            self.save_rois_and_export(ask_overwrite=False)
        self.printc('DONE')
