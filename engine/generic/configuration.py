...
class Config(object):
    ...
    def todict(self):
        ...
        for parameter_name in [class_variable for class_variable in dir(self) if class_variable.isupper() or 'user' == class_variable]:
                if type(getattr(self,parameter_name)).__name__=='interp1d':
                    packed2dict[parameter_name]=0
                elif hasattr(getattr(self,parameter_name),  'dtype') and getattr(self,parameter_name).dtype.names !=None and  'row' in getattr(self,parameter_name).dtype.names:
