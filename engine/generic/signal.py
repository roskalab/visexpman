...
def shape2distance(im,iterations):
    ...
    if iterations>max_iterations:
        steps=numpy.linspace(1,iterations,max_iterations,dtype=numpy.int)
    else:
        steps=numpy.arange(iterations)
    steps=list(steps)
    for i in steps:
        if steps.index(i)==0:
            it=i
        else:
            it=i-steps[steps.index(i)-1]
        input=scipy.ndimage.morphology.binary_erosion(input,iterations=it)
