...
class SpotOnOff(experiment.ExperimentConfig):
    def _create_parameters(self):
        self.SPOT_SIZE = 4000  # fullfield 2500
        self.ON_TIME = 1.86 #s
        self.OFF_TIME = 1.86 #s
        ...
        self.POST_STIMULUS_WAIT=5.0
        self.BACKGROUND_TIME = self.INITIAL_DELAY
...
class SpotExperiment(experiment.Experiment):
    def run(self):
        for repetitions in range(self.experiment_config.REPETITIONS_ALL):
            ...
            if self.abort:
                break
        self.show_fullscreen(duration = self.experiment_config.POST_STIMULUS_WAIT, color =  self.experiment_config.BACKGROUND_COLOR)
