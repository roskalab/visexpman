...
class GratParametersCo(experiment.ExperimentConfig):
    def _create_parameters(self):
        self.SPATIAL_FREQUENCY = [0.0025, 0.004, 0.01, 0.02] #[0.0013, 0.0025, 0.004, 0.01, 0.02] [0.0013,  0.0025,  0.004,  0.01,  0.02,  0.0398,  0.0794,  0.156] 0.01
        # random.shuffle(self.SPATIAL_FREQUENCY)      
        self.TEMPORAL_FREQUENCY = [0.15, 0.4085,  0.6742,  1.1126,  1.8361,  3.03] # [0.15,  0.2475,  0.4085,  0.6742,  1.1126,  1.8361,  3.03,  5.0] 0.6742
        ...
        self.GRATING_ANGLE = [0, 180, 90, 270] #degrees [0, 45, 90, 135, 180, 225, 270, 315]
        self.GRATING_SIZE = utils.cr((0, 0))
        self.GRATING_OFFSET = 1.0 #grating presented without movement
        self.GRATING_PAUSE = 2 #background presentation between gratings
        self.GRATING_DURATION = 8.0/2 #s, presentation of moving grating                           
...
class GratExperiment(experiment.Experiment):
    def prepare(self):
        list_names = ['SPATIAL_FREQUENCY', 'TEMPORAL_FREQUENCY', 'GRATING_MID_CONTRAST', 'GRATING_CONTRAST', 'GRATING_ANGLE']
        n_iterations = self.experiment_config.REPETITIONS
        for pn in list_names:
            n_iterations *= len(getattr(self.experiment_config, pn))
        self.fragment_durations = [n_iterations * (self.experiment_config.GRATING_OFFSET+self.experiment_config.GRATING_DURATION+self.experiment_config.GRATING_PAUSE)]
