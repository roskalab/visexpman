...
class contrastStepsParameters(experiment.ExperimentConfig):
    def _create_parameters(self):
        self.SPOT_SIZE = 4000  # fullfield 2500
        self.ON_TIME = 2 #s
        self.OFF_TIME = 2 #s
        self.BACKGROUND_TIME = 2
        self.SPOT_CONTRAST_ON = [0.625, 0.75, 0.875, 0.999]  #[0.625, 0.75, 0.87, 0.999]  #N [0.999]
        self.SPOT_CONTRAST_OFF = [0.5, 0.375, 0.25, 0.125]
...
class ContrastExperiment(experiment.Experiment):
    def run(self):
        for repetitions in range(self.experiment_config.REPETITIONS_ALL):
            ...
            for spot_contrast_on in self.experiment_config.SPOT_CONTRAST_ON:
                ...
                #self.show_shape(shape = 'rect',  duration = self.experiment_config.OFF_TIME,  color = spot_contrast_off, 
                  #                      background_color = self.experiment_config.BACKGROUND_COLOR,  size = self.experiment_config.SPOT_SIZE)
                #self.show_fullscreen(duration = self.experiment_config.BACKGROUND_TIME, color =  self.experiment_config.BACKGROUND_COLOR)
                self.show_shape(shape = 'rect',  duration = self.experiment_config.ON_TIME,  color = spot_contrast_on, 
                                        background_color = self.experiment_config.BACKGROUND_COLOR,  size = self.experiment_config.SPOT_SIZE)
                #self.show_fullscreen(duration = self.experiment_config.BACKGROUND_TIME, color =  self.experiment_config.BACKGROUND_COLOR)
                if self.abort:
                    break
