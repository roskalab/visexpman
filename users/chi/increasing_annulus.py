    ...
    def run(self):
        for color in self.experiment_config.COLORS:
            self.flash_stimulus('a', [self.experiment_config.ON_TIME, self.experiment_config.OFF_TIME], color, sizes = self.experiment_config.SIZE, 
                    background_color = self.experiment_config.BACKGROUND, position = utils.rc((0,  0)),block_trigger = True, 
                    ring_sizes = 0.5*(self.experiment_config.SIZE - numpy.array(self.experiment_config.INNER_CIRCLE_SIZES)))
