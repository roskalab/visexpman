...
from visexpman.generic import utils,stringop,fileop,signal,introspect
...
def load_experiment_config_names(config, widget):#OBSOLETE
    ...
    if hasattr(config, 'user'):
        import visexpman
        experiment_config_list = utils.fetch_classes('visexpman.users.' + config.user,  required_ancestors = visexpman.vision_experiment.experiment.ExperimentConfig, direct = False)
