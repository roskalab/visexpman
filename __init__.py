...
# from here init.py was copied from engine folder
import argparse, warnings, platform, sys, multiprocessing

from visexpman.generic import utils, fileop, log, introspect
...
def application_init(**kwargs):
    ...
    if len(kwargs) < 3:  # Find user, machine config and application name from command line parameters
        ...
        argparser.add_argument('-a', '--user_interface_name',
                               help='Application to be started: main_ui, stim, ca_imaging')
        ...
        argparser.add_argument('--vu',
                               help='Use visexpu module instead of visexpman.users for loading machine configs and stimuli')
    ...
    if len(config_class) == 0:  # Try user's folder if not found in common folder
        ...
        if len(config_class) == 0:  # Machine config class not found
            raise RuntimeError(
                '{0} user\'s {1} machine configuration class cannot be found'.format(args['user'], args['config']))
        ...
        if '_PATH' == parname[-5:]:
            ...
            if free_space < machine_config.FREE_SPACE_ERROR_THRESHOLD:
                raise FreeSpaceError('No free space on {0}. Only {1} MB is available.'
                                     .format(getattr(machine_config, parname),
                                             fileop.free_space(getattr(machine_config, parname)) / 2 ** 20))
                ...
                warnings.warn('Running out of free space on {0} ({1}). Only {2} MB is available.'
                              .format(getattr(machine_config, parname), parname,
                                      fileop.free_space(getattr(machine_config, parname)) / 2 ** 20))
    ...
    sockets = queued_socket.start_sockets(machine_config.user_interface_name, machine_config, logger,
                                          kwargs.get('enable_sockets', True))
    ...
    if machine_config.PLATFORM == 'ao_cortical' and args['user_interface_name'] == 'stim':
        ...
        context['mes_socket'] = network_interface.start_client(machine_config, 'STIM', 'STIM_MES',
                                                               queue_in=mes_response, queue_out=mes_command)
...
class TestApplicationInit(unittest.TestCase):
    ...
    @unittest.skipIf(platform.system() == 'Windows' and 'unittest_aggregator' in sys.argv[0],
                     'Does not work on windows system')
    ...
    def test_02_no_command_line_args(self):
        self.context = application_init(user='test', config='GUITestConfig', user_interface_name='main_ui',
                                        log_start=True)

    #    @unittest.skip('')
    def test_03_invalid_config(self):
        self.assertRaises(RuntimeError, application_init, user='test', config='GUITestConfig1',
                          user_interface_name='main_ui', log_start=True)
    ...
    def test_04_freespace_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            self.context = application_init(user='test', config='AppInitTest4Config', user_interface_name='main_ui',
                                            log_start=True)
            self.assertEqual('Running out of free space on' in str(w[-1].message), True)

    #    @unittest.skip('')
    def test_05_freespace_error(self):
        self.assertRaises(FreeSpaceError, application_init, user='test', config='AppInitTest5Config',
                          user_interface_name='main_ui', log_start=True)
